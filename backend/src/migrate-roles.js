  ...
  try {
    ...
    if (usersToMigrate.length > 0) {
      ...
      usersToMigrate.forEach((user) => {
        console.log(
          `   - ${user.nombre} (${user.email}): ${user.role} → operador`,
        );
      });
      console.log();
    }
    ...
    const countIndep = usersToMigrate.filter(
      (u) => u.role === "operador_independiente",
    ).length;
    ...
    const countAgencia = usersToMigrate.filter(
      (u) => u.role === "operador_agencia",
    ).length;
    ...
    if (usersWithoutPhone.length > 0) {
      console.log(
        `⚠️  ADVERTENCIA: ${usersWithoutPhone.length} usuarios sin teléfono:`,
      );

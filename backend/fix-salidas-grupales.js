...
async function fixSalidasGrupales() {
  try {
    ...
    const possibleNames = ['salidas_grupales', 'SalidasGrupales', 'SalidaGrupals', 'SalidaGrupales'];
    ...
    for (const tableName of possibleNames) {
      ...
      try {
        ...
        if (info.length > 0) {
          console.log(`   ✅ Tabla ${tableName} existe con ${info.length} columnas`);
          
          // Agregar columnas faltantes
          const columns = [
            { name: 'userId', sql: `ALTER TABLE ${tableName} ADD COLUMN userId INTEGER` },
            { name: 'isPublic', sql: `ALTER TABLE ${tableName} ADD COLUMN isPublic BOOLEAN DEFAULT 0` },
            { name: 'activo', sql: `ALTER TABLE ${tableName} ADD COLUMN activo BOOLEAN DEFAULT 1` },
          ];

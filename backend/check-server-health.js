...
async function runTests() {
  ...
  if (invalidLogin.isJson && invalidLogin.status === 401) {
    ...
    if (!invalidLogin.isJson) {
      console.log(
        `      ⚠️  CRÍTICO: El servidor está devolviendo HTML en lugar de JSON`,
      );
  ...
  if (failed === 0) {
    ...
    console.log("\n💡 Soluciones comunes:");
    console.log(
      "   1. Verificar que el archivo .env existe y tiene JWT_SECRET",
    );
    ...
    console.log(
      "   5. Verificar que .htaccess está configurado correctamente\n",
    );
    process.exit(1);
  }
}

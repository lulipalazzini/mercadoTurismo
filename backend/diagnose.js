...
const envVars = ['NODE_ENV', 'PORT', 'DATABASE_URL', 'JWT_SECRET', 'FRONTEND_URL'];
for (const envVar of envVars) {
  const value = process.env[envVar];
  console.log(`   ${value ? '✅' : '⚠️ '} ${envVar}: ${value ? '(configurada)' : 'NO configurada'}`);
}
...
console.log('\n7️⃣  Controladores:');
try {
  const paquetesController = await import('./src/controllers/paquetes.controller.js');
  console.log('   ✅ paquetes.controller.js');
  console.log(`      Funciones exportadas: ${Object.keys(paquetesController).join(', ')}`);
} catch (error) {

...
if (fs.existsSync(envPath)) {
  ...
  envVars.forEach((varName) => {
    ...
    console.log(`   ${hasVar ? "✅" : "⚠️ "} ${varName}: ${hasVar ? (isEmpty ? "Definido" : "VACÍO") : "NO DEFINIDO"}`);
  });
} else {
...
try {
  ...
  const requiredDeps = ["express", "sequelize", "sqlite3", "jsonwebtoken", "bcryptjs"];
...
if (fs.existsSync(dbPath)) {
  ...
  console.log("   ⚠️  Archivo database.sqlite NO existe (se creará en primer inicio)");
}
